...
/* v2.x: userspace sends {pathname, err} (no target_ino); we resolve ino internally,
 * store in v1.5.5 hlist, and set INODE_STATE_SUS_PATH via the existing helper. */
void susfs_add_sus_path(void __user **user_info) {
	...
	if (update_hlist) {
		SUSFS_LOGI("target_ino: '%lu', target_pathname: '%s' is successfully updated to SUS_PATH_HLIST\n",
				target_ino, new_entry->target_pathname);
	} else {
...
bool susfs_hide_sus_mnts_for_non_su_procs = true; /* inert: v1.5.5 hooks gate on per-path INODE_STATE_SUS_MOUNT */
...
/* v2.x: userspace sends {pathname, target_dev, err}; target_dev provided by tool */
...
/* v2.x (0x55561): inert global toggle; v1.5.5 hooks ignore it (per-path flag set by 0x55560) */
...
/* v2.x: userspace sends v2100 struct (interleaved nsec + flags + err); target_ino provided by tool.
 * Translate field-by-field into v1.5.5 internal info (nsec order differs; drop flags). NO memcpy. */
void susfs_add_sus_kstat(void __user **user_info) {
	...
	/* translate v2.x userspace -> v1.5.5 internal info field-by-field */
	new_entry->info.is_statically = info.is_statically;
	new_entry->info.target_ino = info.target_ino;
	strncpy(new_entry->info.target_pathname, info.target_pathname, SUSFS_MAX_LEN_PATHNAME-1);
	...
	/* info.flags intentionally dropped: v1.5.5 hook spoofs all fields unconditionally */
...
/* v2.x: find by pathname, update target_ino (+ spoofed_size/blocks if provided). Input field
 * names (target_ino, spoofed_size, spoofed_blocks) match v1.5.5 internal info names. */
	...
	hash_for_each_safe(SUS_KSTAT_HLIST, bkt, tmp_node, tmp_entry, node) {
		if (!strcmp(tmp_entry->info.target_pathname, info.target_pathname)) {
			...
			if (info.spoofed_size > 0) {
				SUSFS_LOGI("updating spoofed_size for pathname: '%s' in SUS_KSTAT_HLIST\n", info.target_pathname);
				new_entry->info.spoofed_size = info.spoofed_size;
			}
			if (info.spoofed_blocks > 0) {
				SUSFS_LOGI("updating spoofed_blocks for pathname: '%s' in SUS_KSTAT_HLIST\n", info.target_pathname);
				new_entry->info.spoofed_blocks = info.spoofed_blocks;
			}
			hash_del(&tmp_entry->node);
			kfree(tmp_entry);
...
/* v2.x: userspace sends {pathname, mnt_mode, err} */
...
/* susfs_run_try_umount_for_current_mnt_ns() is defined in fs/namespace.c (session-18 port) */
...
/* v2.x: userspace sends {release, version, err} */
...
/* enable_log */
#ifdef CONFIG_KSU_SUSFS_ENABLE_LOG
/* v2.x: userspace sends {bool enabled, int err} (replaces v1.5.5 raw-scalar susfs_set_log) */
...
/* v2.x: userspace sends {char[8192], int err}; string at offset 0 */
...
/* v2.x: userspace sends v2100 {target_pathname, redirected_pathname, uid_scheme, err} (no target_ino).
 * Resolve target_ino internally; uid_scheme accepted but ignored (v1.5.5 hook gates uid<2000 fixed). */
void susfs_add_open_redirect(void __user **user_info) {
	...
	/* info.uid_scheme intentionally ignored: v1.5.5 hook susfs_get_redirected_path gates on uid<2000 fixed */
...
/* sus_su (config-off in this build; kept v1.5.5 verbatim) */
...
int susfs_sus_su(struct st_susfs_sus_su* __user user_info) {
	struct st_susfs_sus_su info;
	int last_working_mode = susfs_sus_su_working_mode;

	if (copy_from_user(&info, user_info, sizeof(struct st_susfs_sus_su))) {
...
/* v2.x: returns newline-joined string of enabled CONFIG_KSU_SUSFS_* names + err=0 (replaces v1.5.5 u64 bitmask) */
...
/* show version (THE probe fix: writes "v2.2.0" + err=0; tool locks ABI_v2000 iff err==0) */
